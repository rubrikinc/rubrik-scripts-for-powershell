...
$get_cred = $true # if set to $true, this will ignore $rubrik_user and $rubrik_pass variables, set to $false to use the credentials above. This would be useful for automating the run of the script
$output_type = 'html' # enter 'html' for an html report, 'csv' for a CSV based-report, or 'text' for a plain text report
$output_folder = '.' # thiscan  be modified to 'C:\temp' or whatever is required, leave as '.' to write to script path, do not include trailing slash on folder path
# Do not change anything after this point
if ($get_cred) {
    $rk_cred = Get-Credential
}
...
if ($get_cred) {
    $rk_connection = Connect-Rubrik -Server $rubrik_cluster -Credential $rk_cred
} else {
...
foreach ($rk_sla in $rk_all_slas) {
    ...
    if ($rk_sla.replicationSpecs -ne $null) {
        ...
        if ($get_cred) {
            $rep_target_info = Invoke-WebRequest -Uri $("https://"+$rubrik_cluster+"/api/internal/replication/target/"+$rk_sla.replicationSpecs.locationId) -Headers $headers -Method GET -Credential $rk_cred
        } else {
    ...
    if ($rk_sla.archivalSpecs -ne $null) {
        ...
        if ($get_cred) {
            $archive_locations = Invoke-WebRequest -Uri $("https://"+$rubrik_cluster+"/api/internal/archive/location") -Headers $headers -Method GET -Credential $rk_cred
        } else {
